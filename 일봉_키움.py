...
class KiwoomAPI(QMainWindow):
    ...
    def after_login(self):
        self.req_opt10081("005930_AL", date=datetime.datetime.now().strftime("%Y%m%d"))
    ...
    def req_opt10081(self, code, date=None):
        if date is None:
    ...
    # 조회 데이터는 이 이벤트에서 GetCommData 메서드를 사용하여 가져올 수 있습니다.
        if rq_name == "opt10081_req":
            self.on_opt10081_req(tr_code,rq_name)

    def on_opt10081_req(self, tr_code, rq_name):
